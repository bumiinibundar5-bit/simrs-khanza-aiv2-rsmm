...
public final class RMPemeriksaanEEG extends javax.swing.JDialog {
    ...
    private void cetakPreview() {
        ...
        try {
            ...
            sb.append("<!DOCTYPE html><html lang='id'><head>")
              .append("<meta charset='UTF-8'>")
              .append("<meta name='viewport' content='width=device-width, initial-scale=1.0'>")
              .append("<title>Hasil Pemeriksaan EEG - ").append(namaPasien).append("</title>")
              .append("<style>")
              .append("  * { margin:0; padding:0; box-sizing:border-box; }")
              .append("  body { font-family: 'Arial', sans-serif; font-size: 11px; color: #222; background: #f0f0f0; }")
              .append("  .page { background: white; width: 210mm; min-height: 297mm; margin: 10px auto; padding: 15mm 15mm 10mm 15mm; box-shadow: 0 2px 8px rgba(0,0,0,0.2); }")
              .append("  .kop { text-align: center; border-bottom: 3px double #333; padding-bottom: 8px; margin-bottom: 10px; }")
              .append("  .kop h2 { font-size: 14px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; }")
              .append("  .kop h3 { font-size: 12px; font-weight: bold; }")
              .append("  .kop p  { font-size: 10px; color: #555; }")
              .append("  .judul-form { text-align: center; font-size: 13px; font-weight: bold; background: #e8f4e8; padding: 5px; border: 1px solid #aaa; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }")
              .append("  table.identitas { width: 100%; border-collapse: collapse; margin-bottom: 8px; }")
              .append("  table.identitas td { padding: 3px 5px; font-size: 11px; vertical-align: top; }")
              .append("  table.identitas td.lbl { width: 120px; font-weight: bold; color: #444; }")
              .append("  table.identitas td.sep { width: 8px; }")
              .append("  .section { margin-bottom: 8px; }")
              .append("  .section-title { background: #2e6e2e; color: white; padding: 4px 8px; font-size: 11px; font-weight: bold; text-transform: uppercase; margin-bottom: 4px; }")
              .append("  table.data { width: 100%; border-collapse: collapse; font-size: 10.5px; }")
              .append("  table.data td { padding: 3px 6px; border: 1px solid #ccc; vertical-align: top; }")
              .append("  table.data td.lbl { background: #f5f5f5; font-weight: bold; width: 35%; }")
              .append("  table.data td.val { width: 65%; }")
              .append("  table.data td.lbl2 { background: #f5f5f5; font-weight: bold; width: 20%; }")
              .append("  table.data td.val2 { width: 30%; }")
              .append("  .badge { display: inline-block; padding: 1px 7px; border-radius: 3px; font-weight: bold; font-size: 10.5px; }")
              .append("  .badge-normal { background: #d4edda; color: #155724; border: 1px solid #c3e6cb; }")
              .append("  .badge-abnormal { background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }")
              .append("  .badge-info { background: #d1ecf1; color: #0c5460; border: 1px solid #bee5eb; }")
              .append("  .kesimpulan-box { border: 2px solid #2e6e2e; padding: 8px; background: #f0f8f0; margin: 4px 0; font-size: 12px; }")
              .append("  .ttd { margin-top: 20px; }")
              .append("  table.ttd-tbl { width: 100%; }")
              .append("  table.ttd-tbl td { text-align: center; padding: 5px; }")
              .append("  .print-btn { position: fixed; top: 15px; right: 15px; background: #2e6e2e; color: white; border: none; padding: 10px 20px; font-size: 13px; border-radius: 5px; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.3); z-index: 999; }")
              .append("  .print-btn:hover { background: #245424; }")
              .append("  @media print { .print-btn { display: none; } body { background: white; } .page { box-shadow: none; margin: 0; width: 100%; } }")
              .append("</style></head><body>")

              // Tombol Print
              .append("<button class='print-btn' onclick='window.print()'>&#128438; Cetak</button>")

              .append("<div class='page'>")

              // KOP
              .append("<div class='kop'>")
              .append("<h2>").append(namaRs).append("</h2>")
              .append("<h3>LABORATORIUM EEG - SUBDIVISI NEUROFISIOLOGI</h3>")
              .append("<p>POLIKLINIK BRAIN CENTRE</p>")
              .append("</div>")

              // Judul Form
              .append("<div class='judul-form'>HASIL PEMERIKSAAN ELEKTROENSEFALOGRAFI (EEG)</div>")

              // Identitas Pasien
              .append("<div class='section'>")
              .append("<div class='section-title'>Identitas Pasien</div>")
              .append("<table class='identitas'><tr>")
              .append("<td class='lbl'>Nama Pasien</td><td class='sep'>:</td><td><b>").append(namaPasien).append("</b></td>")
              .append("<td class='lbl'>No. Rekam Medis</td><td class='sep'>:</td><td>").append(noRM).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Jenis Kelamin</td><td class='sep'>:</td><td>").append(jk).append("</td>")
              .append("<td class='lbl'>No. Rawat</td><td class='sep'>:</td><td>").append(noRawat).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Tanggal Lahir</td><td class='sep'>:</td><td>").append(tglLahir).append("</td>")
              .append("<td class='lbl'>Tgl. Rekaman</td><td class='sep'>:</td><td>").append(tglRekaman).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Dokter Pengirim</td><td class='sep'>:</td><td>").append(dokterPengirimVal).append("</td>")
              .append("<td class='lbl'>Diagnosa</td><td class='sep'>:</td><td>").append(diagnosa).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Operator EEG</td><td class='sep'>:</td><td>").append(nmOperator).append("</td>")
              .append("<td class='lbl'>Dokter Pemeriksa</td><td class='sep'>:</td><td><b>").append(nmDokter).append("</b></td>")
              .append("</tr></table>")
              .append("</div>")

              // Interpretasi
              .append("<div class='section'>")
              .append("<div class='section-title'>Interpretasi EEG</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>Keadaan Rekaman</td><td class='val'>").append(keadaan).append("</td>")
              .append("<td class='lbl2'>Bervoltase</td><td class='val2'>").append(bervoltase).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Frekuensi</td><td class='val' colspan='3'>").append(frekuensi).append("</td>")
              .append("</tr></table>")
              .append("</div>")

              // Temuan Gelombang
              .append("<div class='section'>")
              .append("<div class='section-title'>Temuan - Temuan Yang Tampak</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>Gelombang Patologis</td><td class='val'>").append(gelPat).append("</td>")
              .append("<td class='lbl2'>Gelombang Lambat</td><td class='val2'>").append(gelLambat).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>GL Bervoltase</td><td class='val'>").append(glVolt).append("</td>")
              .append("<td class='lbl2'>GL Frekuensi</td><td class='val2'>").append(glFreq).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>GL Localized</td><td class='val'>").append(glLocTipe).append(" di daerah ").append(glLocDaerah).append("</td>")
              .append("<td class='lbl2'>GL Generalized</td><td class='val2'>").append(glGenTipe).append(" pada kedua hemisfer</td>")
              .append("</tr></table>")
              .append("</div>")

              // IED
              .append("<div class='section'>")
              .append("<div class='section-title'>IED (Interictal Epileptiform Discharges)</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>IED</td><td class='val'>").append(iedAda).append("</td>")
              .append("<td class='lbl2'>Bervoltase</td><td class='val2'>").append(iedVolt).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>Bentuk</td><td class='val'>").append(iedBentuk)
              .append(iedKet.isEmpty() ? "" : " (" + iedKet + ")").append("</td>")
              .append("<td class='lbl2'>&nbsp;</td><td class='val2'>&nbsp;</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>IED Localized</td><td class='val'>").append(iedLocTipe).append(" di daerah ").append(iedLocDaerah).append("</td>")
              .append("<td class='lbl2'>IED Generalized</td><td class='val2'>").append(iedGenTipe).append(" pada kedua hemisfer</td>")
              .append("</tr></table>")
              .append("</div>")

              // Temuan Lain
              .append("<div class='section'>")
              .append("<div class='section-title'>Temuan Lain</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl2'>Hips Aritmia</td><td class='val2'>").append(hips).append("</td>")
              .append("<td class='lbl2'>Vertex Transient</td><td class='val2'>").append(vertex).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl2'>Sleep Spindle</td><td class='val2'>").append(sleep).append("</td>")
              .append("<td class='lbl2'>Respon Tutup/Buka Mata</td><td class='val2'>").append(responMata).append("</td>")
              .append("</tr></table>")
              .append("</div>")

              // Artefak
              .append("<div class='section'>")
              .append("<div class='section-title'>Artefak</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>Artefak</td><td class='val' colspan='3'>").append(artefak).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl2'>Kontraksi Otot</td><td class='val2'>").append(artKont).append("</td>")
              .append("<td class='lbl2'>Denyutan Pembuluh</td><td class='val2'>").append(artDeny).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl2'>Keringat</td><td class='val2'>").append(artKer).append("</td>")
              .append("<td class='lbl2'>Kedipan Mata</td><td class='val2'>").append(artKed).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl2'>Gerakan Bola Mata</td><td class='val2'>").append(artGerak).append("</td>")
              .append("<td class='lbl2'>Gangguan Mekanis</td><td class='val2'>").append(artMek).append("</td>")
              .append("</tr></table>")
              .append("</div>")

              // HV/PS
              .append("<div class='section'>")
              .append("<div class='section-title'>Hiperventilasi / Fotik Stimulasi (HV/PS)</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>HV/PS</td><td class='val'>").append(hvps).append("</td>")
              .append("<td class='lbl2'>Keterangan</td><td class='val2'>").append(hvKet.isEmpty() ? "-" : hvKet).append("</td>")
              .append("</tr></table>")
              .append("</div>")

              // Kesimpulan
              .append("<div class='section'>")
              .append("<div class='section-title'>Kesimpulan</div>")
              .append("<div class='kesimpulan-box'>")
              .append("<span class='badge ").append(kesimpulan.contains("Normal") ? "badge-normal" : "badge-abnormal").append("'>")
              .append(kesimpulan).append("</span>")
              .append("</div>")
              .append("<table class='data'><tr>")
              .append("<td class='lbl'>a. IED</td><td class='val'>").append(kesAIED)
              .append("</td><td class='lbl2'>Slow Wave</td><td class='val2'>").append(kesASW).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>di Daerah</td><td class='val' colspan='3'>").append(kesADaerah.isEmpty() ? "-" : kesADaerah).append("</td>")
              .append("</tr><tr>")
              .append("<td class='lbl'>b. IED</td><td class='val'>").append(kesBIED)
              .append("</td><td class='lbl2'>Slow Wave</td><td class='val2'>").append(kesBSW).append(" pada kedua hemisfer</td>")
              .append("</tr></table>")
              .append("</div>")

              // Usul Komentar
              .append("<div class='section'>")
              .append("<div class='section-title'>Usul / Komentar</div>")
              .append("<table class='data'><tr>")
              .append("<td style='padding:6px;'>").append(usulKom.isEmpty() ? "-" : usulKom).append("</td>")
              .append("</tr></table>")
              .append("</div>")

              // TTD dengan QR Code Base64
              .append("<div class='ttd'>");

            // Generate QR Code Base64 untuk dokter pemeriksa
            String qrText = "Ditandatangani secara elektronik oleh : " + nmDokter;
            String qrBase64 = generateQrBase64(qrText, 120);

            sb.append("<table class='ttd-tbl'><tr>")
              .append("<td style='width:60%'></td>")
              .append("<td style='text-align:center; padding:5px;'>")
              .append("Makassar, ").append(tglRekaman.substring(0, 10)).append("<br/>")
              .append("Yang Memeriksa,<br/><br/>");

            if (qrBase64 != null) {
                sb.append("<img src='data:image/png;base64,").append(qrBase64).append("' ")
                  .append("width='100' height='100' style='display:block;margin:0 auto 4px auto;' ")
                  .append("title='").append(qrText).append("' />");
            } else {
                sb.append("<div style='width:100px;height:100px;border:1px dashed #aaa;margin:0 auto 4px auto;")
                  .append("display:flex;align-items:center;justify-content:center;font-size:9px;color:#888;'>")
                  .append("TTD Elektronik</div>");
            }

            sb.append("<b>Dr. ").append(nmDokter).append("</b>")
              .append("</td></tr></table>")
              .append("</div>")
